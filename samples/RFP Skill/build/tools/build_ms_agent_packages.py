...
rfp-automation-kit catalog, which stays the single source of truth.
    ...
    python build/tools/build_ms_agent_packages.py --keep-tree
...
# <sample>/build/tools/this.py  ->  parents[2] is the sample root.
SAMPLE = Path(__file__).resolve().parents[2]
REPO = SAMPLE  # paths are printed relative to the sample folder
# The Copilot CLI marketplace registers plugins by repo-relative path, and a
# path containing a space is a liability for CLI tooling — so the plugin is
# emitted to a space-free directory at the repository root.
REPO_ROOT = Path(__file__).resolve().parents[4]
COPILOT_PLUGIN_DIR = REPO_ROOT / "plugins" / "rfp-response"
SKILL_SRC = SAMPLE / "skill" / "rfp-response"
KIT = SAMPLE / "build" / "rfp-automation-kit"
...
def build_copilot_plugin(flat: Path) -> int:
    """Emit a GitHub Copilot CLI plugin from the built skill.

    Copilot CLI installs plugins from a marketplace repo: `marketplace.json`
    lists each plugin by a repo-relative `source` path, and that directory holds
    a `plugin.json` manifest alongside the content. Skills sit in `skills/<name>/`
    beside the manifest — the same layout Claude Code uses, which is why one
    folder serves both.

    Generated and committed like the ZIPs, so `copilot plugin marketplace add
    OWNER/REPO` works against a fresh clone with no build step.
    """
    ...
    dest = COPILOT_PLUGIN_DIR
    if dest.exists():
        shutil.rmtree(dest)
    (dest / "skills").mkdir(parents=True)
    shutil.copytree(flat, dest / "skills" / "rfp-response")
    ...
    manifest = {
        ...
        "author": {"name": "AgentDudeSamples"},
        "repository": "https://github.com/Dewain27/AgentDudeSamples",
        "license": "MIT",
        "keywords": ["rfp", "proposal", "bid", "tender", "sales",
                     "document-generation", "agent-skills"],
    }
    (dest / "plugin.json").write_text(_json.dumps(manifest, indent=2) + "\n",
                                      encoding="utf-8")

    readme = dest / "README.md"
    readme.write_text(f"""# rfp-response
...
""", encoding="utf-8")
    ...
    print(f"\nstaged {dest.relative_to(REPO_ROOT)}  (GitHub Copilot CLI plugin)")
    print(f"  files           : {n}")
    print(f"  manifest        : plugin.json (v{PLUGIN_VERSION})")
    print("Copilot CLI: copilot plugin marketplace add Dewain27/AgentDudeSamples")
    return 0
...
def main():
    ...
    if args.target in ("copilot-plugin", "all"):
        rc |= build_copilot_plugin(stage)
