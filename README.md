...
| **[Build Work Estimator](samples/Build%20Work%20Estimator/)** | An Agent Skill that estimates the work of **building** something with an AI coding agent — turns, tokens, and cost — calibrated from your own session history rather than assumed constants. Requires a contingency reserve and checks whether it actually covers observed variance, translates Microsoft work into build-time Copilot Credits, and learns from recorded actuals. Ships a full financial-services scenario with a complete technical specification. Estimates the build, never the run. | GitHub Copilot CLI · Claude Code · Copilot Cowork · Claude Cowork · Copilot Studio |
